...
class Game():
	def __init__(self):
		...
		self.end_msg = self.font.render('Has Perdido!', False, BLACK)
		self.end_msg_x, self.end_msg_y, _, _ = self.end_msg.get_rect(center = DISP_CENTER)

	

		self.restart_msg = self.font.render('Presiona R Para Volver a Empezar o Presiona Ctrl para salir', False, BLACK)
...
class Background():
	...
	def getImage(self):
		...
		bg = pg.image.load('images/bg.jpg').convert()
